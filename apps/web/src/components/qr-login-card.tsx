...
export function QrLoginCard() {
  ...
  return (
    <div className="w-full max-w-sm rounded-xl border border-border bg-surface p-6 text-center">
      {state.phase === "loading" && <StatusText>Requesting a secure code…</StatusText>}

      {state.phase === "pending" && <PendingView token={state.token} expiresAt={state.expiresAt} />}

      {state.phase === "approved" && <StatusText>Approved — signing you in…</StatusText>}
      ...
      {state.phase === "denied" && <TerminalView message="Login request denied." onRetry={restart} />}
      {state.phase === "expired" && <TerminalView message="This code expired." onRetry={restart} />}
      {state.phase === "error" && <TerminalView message={state.message} onRetry={restart} />}
    </div>
  );
}
...
function PendingView({ token, expiresAt }: { token: string; expiresAt: string }) {
  ...
  useEffect(() => {
    let cancelled = false;
    QRCode.toDataURL(buildLoginDeepLink(token), { margin: 1, width: 220 }).then((url) => {
  ...
  return (
    <div>
      <p className="text-sm text-foreground-muted">Scan this with NDYAPPS to log in</p>
      <div className="mx-auto mt-4 flex h-[220px] w-[220px] items-center justify-center rounded-lg bg-white p-2">
        {qrDataUrl ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={qrDataUrl} alt="NDY HUB login QR code" width={204} height={204} />
        ) : (
...
function TerminalView({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
    <div>
