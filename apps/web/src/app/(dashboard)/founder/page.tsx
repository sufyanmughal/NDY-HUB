...
import {
  getFounderEcosystemOverview,
  ApiError,
  type FounderEcosystemOverview,
} from "@/lib/api";
...
export default function FounderMissionControlPage() {
  const { auth } = useAuth();
  const [overview, setOverview] = useState<FounderEcosystemOverview | null>(
    null,
  );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-foreground-muted">
          The command center for the NDJOYIT ecosystem — health, users, and
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          ...
          <StatTile
            ...
            value={
              overview ? overview.users.activeSessions.toLocaleString() : "…"
            }
            icon={Activity}
          />
          <StatTile
            ...
            value={
              overview
                ? overview.users.newVerificationsToday.toLocaleString()
                : "…"
            }
            icon={ShieldCheck}
          />
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 lg:grid-cols-4">
          <StatTile
            ...
            value={
              overview ? overview.memberships.active.toLocaleString() : "…"
            }
            icon={CreditCard}
          />
          <StatTile
          ...
          <StatTile
            ...
            value={
              overview
                ? `${overview.cryndy.salesToday.count} · ${formatCents(overview.cryndy.salesToday.amountCents)}`
                : "…"
            }
            icon={Coins}
          />
          <StatTile
            ...
            value={
              overview ? overview.ndybits.issuedToday.toLocaleString() : "…"
            }
            icon={Boxes}
          />
        </div>
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 lg:grid-cols-4">
          <StatTile
            ...
            value={
              overview
                ? `${overview.cryndy.salesAllTime.count} · ${formatCents(overview.cryndy.salesAllTime.amountCents)}`
                : "…"
            }
            icon={Coins}
          />
          <StatTile
            ...
            value={
              overview ? overview.ndybits.issuedAllTime.toLocaleString() : "…"
            }
            icon={Boxes}
          />
          <StatTile
            ...
            value={
              overview
                ? overview.systemStatus.database === "ok"
                  ? "Healthy"
                  : "Degraded"
                : "…"
            }
