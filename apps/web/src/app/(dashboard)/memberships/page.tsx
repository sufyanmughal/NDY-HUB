...
const TIER_ORDER: MembershipTier[] = ["RISE", "FLOW", "PULSE", "VAULT", "MODE", "LEGACY"];
...
export default function MembershipsPage() {
  ...
  const refresh = useCallback(() => {
    ...
    getMyMembership(auth.accessToken).then(setSummary).catch((err) => setError((err as Error).message));
  }, [auth]);

  useEffect(() => {
    getMembershipTiers().then(setTiers).catch((err) => setError((err as Error).message));
  }, []);
  ...
  async function handleSubscribe(tier: MembershipTier) {
    ...
    setError(null);
    try {
      const result = await subscribeToTier(accessToken, tier, billingCycle);
  ...
  async function handleCancel(membershipId: string) {
    setError(null);
    try {
      await cancelMembership(accessToken, membershipId);
      refresh();
    } catch (err) {
      setError((err as Error).message);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {summary?.current && (
        <div className="rounded-lg border border-border bg-surface p-5">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-xs uppercase tracking-wide text-foreground-muted">Current plan</div>
              <div className="mt-1 text-lg font-semibold">{summary.current.tierLabel}</div>
            </div>
          ...
          <dl className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-3">
            <div>
              ...
              <dd className="font-mono">{summary.current.billingCycle === "ANNUAL" ? "Annual" : "Monthly"}</dd>
            </div>
            <div>
              <dt className="text-foreground-muted">Started</dt>
              <dd className="font-mono">{new Date(summary.current.startedAt).toLocaleDateString()}</dd>
            </div>
            <div>
              <dt className="text-foreground-muted">Renews</dt>
              <dd className="font-mono">{new Date(summary.current.currentPeriodEnd).toLocaleDateString()}</dd>
            </div>
          </dl>
          <button
      ...
      <div className="flex items-center gap-2">
        ...
        <ToggleButton active={billingCycle === "ANNUAL"} onClick={() => setBillingCycle("ANNUAL")}>
          Annual
        </ToggleButton>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {tiers &&
          TIER_ORDER.map((tier) => {
            ...
            const priceCents = billingCycle === "ANNUAL" ? info.annualPriceCents : info.monthlyPriceCents;
            const isCurrent = summary?.current?.tier === tier;
            return (
              <div key={tier} className="flex flex-col rounded-lg border border-border bg-surface p-5">
                ...
                >
                  {isCurrent ? "Current plan" : busyTier === tier ? "…" : "Subscribe"}
                </button>
              </div>
            );
          })}
      ...
      {summary && summary.history.length > 0 && (
        <div className="rounded-lg border border-border bg-surface p-5">
          <h2 className="text-sm font-medium text-foreground-muted">Membership History</h2>
              ...
              <li key={m.id} className="flex items-center justify-between py-3 text-sm">
                <div>
                  ...
                  <div className="text-xs text-foreground-muted">
                    ...
                    {m.cancelledAt && ` – ${new Date(m.cancelledAt).toLocaleDateString()}`}
                  </div>
                </div>
                <span
                  className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${
                    m.status === "ACTIVE" ? "bg-good/15 text-good" : "bg-foreground-muted/15 text-foreground-muted"
                  }`}
                >
                  {m.status}
                </span>
  ...
  return (
    <button
      ...
      className={`rounded-md px-3 py-1.5 text-sm font-medium ${
        active ? "bg-accent text-white" : "border border-border text-foreground-muted hover:bg-surface-2"
      }`}
    >
      {children}
    </button>
